...
pub async fn create_artifact(
    ...
    if state
        .artifacts
        .find_by_namespace_name(&kind, &req.namespace, &req.name)
        .await?
        .is_some()
    {
    ...
    state
        .events
        .append(DomainEvent::ArtifactCreated {
            artifact_id: artifact.id,
            kind: kind_str.clone(),
            namespace: req.namespace.clone(),
            name: req.name.clone(),
    ...
    Ok((
        StatusCode::CREATED,
        Json(serde_json::json!({ "artifact": artifact, "version": version })),
    ))
}
...
pub async fn update_artifact(
    ...
    if artifact.author_id != claims.sub {
        return Err(ApiError::Forbidden(
            "only the author can update this artifact".into(),
        ));
    }
    ...
    if let Some(content) = req.content {
        ...
        state
            .events
            .append(DomainEvent::ArtifactUpdated {
                artifact_id: artifact.id,
...
pub async fn deprecate_artifact(
    ...
    if artifact.author_id != claims.sub {
        return Err(ApiError::Forbidden(
            "only the author can deprecate this artifact".into(),
        ));
    }
...
pub async fn revoke_artifact(
    ...
    if artifact.author_id != claims.sub {
        return Err(ApiError::Forbidden(
            "only the author can revoke this artifact".into(),
        ));
    }
    ...
    state
        .events
        .append(agentverse_events::types::DomainEvent::ArtifactRevoked {
            artifact_id: artifact.id,
...
pub async fn update_embedding(
    ...
    if artifact.author_id != claims.sub {
        return Err(ApiError::Forbidden(
            "only the author can update the embedding".into(),
        ));
    }
...
pub fn parse_kind(s: &str) -> ApiResult<ArtifactKind> {
    // Accept both singular ("skill") and plural ("skills") REST forms.
    match s {
        "skill" | "skills" => Ok(ArtifactKind::Skill),
        "soul" | "souls" => Ok(ArtifactKind::Soul),
        "agent" | "agents" => Ok(ArtifactKind::Agent),
        "workflow" | "workflows" => Ok(ArtifactKind::Workflow),
        "prompt" | "prompts" => Ok(ArtifactKind::Prompt),
